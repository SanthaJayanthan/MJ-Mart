  ...
  return (
    <div className="no-scrollbar flex-1 h-[95vh] overflow-y-scroll flex flex-col justify-between">
            <form onSubmit={onSubmitHandler} className="md:p-10 p-4 space-y-5 max-w-lg">
                    ...
                    <div className="flex flex-wrap items-center gap-3 mt-2">
                        {Array(4).fill('').map((_, index) => (
                            <label key={index} htmlFor={`image${index}`}>
                                ...
                                <img className="max-w-24 cursor-pointer opacity-100 hover:opacity-50 transition-opacity" src={files[index] ? URL.createObjectURL(files[index]) : assets.upload_icon} alt="uploadArea" width={100} height={100} />
                            </label>
                        ))}
                    </div>
                </div>
                ...
                <button className="px-8 py-2.5 bg-indigo-700 text-white font-medium rounded cursor-pointer opacity-100 hover:opacity-80 transition-opacity">ADD</button>
            </form>
        </div>
