    ...
    return product && (
        <div onClick={()=> {navigate(`/products/${product.category.toLowerCase()}/${product._id}`); scrollTo(0,0)}} className="border border-gray-500/20 rounded-md md:px-4 px-3 py-2 bg-white min-w-56 max-w-56 w-full">
            <div className="group cursor-pointer flex items-center justify-center px-2 h-32 md:h-40">
                <img className="group-hover:scale-105 transition max-h-full max-w-26 md:max-w-36 object-contain" src={product.image?.[0]} alt={product.name} />
            </div>
            <div className="text-gray-500/60 text-sm">
                ...
                <div className="flex items-center gap-0.5">
                    {Array(5).fill('').map((_, i) => (
                            ...
                            <img key={i} className="md:w-3.5 w3" src={i < 4 ? assets.Star_icon2 : assets.star_dull_icon} alt=""/>
